...
* [Jekyll](http://jekyllrb.com/) - программа для генерации статических сайтов
  ...
  - [CSS](http://ru.wikipedia.org/wiki/CSS) - язык описания дизайна Веб-страниц
    + [SASS/SCSS](https://ru.wikipedia.org/wiki/Sass) - улучшенный синтаксис для CSS
...
Если имя сайта задано как `name.ru`, то GitHub Pages автоматически перенаправляет
обращения к домену `www.name.ru` на `name.ru` (и наоборот).
...
Общее количество новостей на главной странице определяется параметром `paginate: ` в YAML-прологе файла `index.html`,
...
Этот файл собирает в формате [RSS](https://ru.wikipedia.org/wiki/RSS) 2.0 новости сайта
для того, чтобы их могли читать агрегаторы новостей.

RSS feed рекомендуется [зарегистрировать](http://www.w3schools.com/rss/rss_publishing.asp)
в каталогах и поисковиках.
...
Здесь размещаются фрагменты HTML-страниц. Фрагмент может быть вставлен в страницу при
помощи директивы Liquid `{% include file.ext %}`.

### [yangshi](/yangshi)

Здесь хранятся таблицы стилей (CSS), определяющие внешний вид страниц.

На данный момент имеется только одна таблица стилей [qi](/yangshi/qi.sass),
но при желании можно добавить и другие.
...
Начиная с версии 2 Jekyll поддерживает автоматическую компиляцию таблиц стилей
формата SASS/SCSS в обычный формат CSS. Для этого необходимо дать файлу расширение
`.sass` или `.scss` и задать YAML-пролог
(хотя бы и пустой, то есть две строчки символов `---`). Тогда этот файл будет
откомпилирован и результат помещён в файл с расширением `.css`.

### [_sass](/_sass)

Здесь размещаются фрагменты таблиц стилей, которые собираются при помощи механизма SASS/SCSS.
В исходном файле могут содержаться директивы вида `@import "name"`, тогда вместо
этой директивы будет подставлено содержимое файла `/_sass/_name.s?ss`.

Это позволяет разбивать большие таблицы стилей на небольшие фрагменты, что облегчает
понимание и сопровождение.

### [js](/js)

Здесь хранятся таблицы программы на языке
[JavaScript](http://ru.wikipedia.org/wiki/JavaScript), которые используются на страницах сайта.
...
Начиная с версии 2 Jekyll поддерживает автоматическую компиляцию
[CoffeeScript](http://ru.wikipedia.org/wiki/CoffeeScript) в JavaScript.
Для этого необходимо дать файлу расширение `.coffee` и задать YAML-пролог
(хотя бы и пустой, то есть две строчки символов `---`). Тогда этот файл будет
откомпилирован и результат помещён в файл с расширением `.js`.
